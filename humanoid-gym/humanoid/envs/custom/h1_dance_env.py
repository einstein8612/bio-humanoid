...
class H1DanceFreeEnv(LeggedRobot):
    ...
    def quaternions_to_euler_array(self, quats: torch.Tensor):
        """
        Convert a batch of quaternions to Euler angles (roll, pitch, yaw).
        
        Args:
            quats (torch.Tensor): A tensor of shape (N, M, 4), where N is the batch size,
                                M is the number of joints, and 4 represents [x, y, z, w].
        
        Returns:
            torch.Tensor: A tensor of shape (N, M, 3), where each quaternion is converted to
                        [roll, pitch, yaw] in radians.
        """
        assert quats.ndim == 3 and quats.shape[-1] == 4, "Input must be of shape (N, M, 4)"
        ...
        # Combine roll, pitch, and yaw into a single tensor
        eulers = torch.stack((roll, pitch, yaw), dim=-1).to(device=self.device)  # Shape: (N, M, 3)
        return eulers
    
    def _local_rotation_to_dof_pos(self, local_rotations: torch.Tensor):
        """
        ...
        eulers_xyz_rpy = self.quaternions_to_euler_array(local_rotations) # Shape: (N, M, 3)
        ref_dof_pos = torch.zeros_like(self.dof_pos, device=self.device)
    ...
    def _local_vel_to_dof_vel(self, local_velocities: torch.Tensor):
        # local_velocities shape: (N, M, 3)
        ref_dof_vel = torch.zeros_like(self.dof_vel, device=self.device)
        
        ref_dof_vel[:, 0] = local_velocities[:, 1, 2]
        ref_dof_vel[:, 1] = local_velocities[:, 1, 0]
        ref_dof_vel[:, 2] = local_velocities[:, 1, 1]
        ref_dof_vel[:, 3] = local_velocities[:, 2, 1]
        ref_dof_vel[:, 4] = local_velocities[:, 3, 1]
        # Right leg
        ref_dof_vel[:, 5] = local_velocities[:, 4, 2]
        ref_dof_vel[:, 6] = local_velocities[:, 4, 0]
        ref_dof_vel[:, 7] = local_velocities[:, 4, 1]
        ref_dof_vel[:, 8] = local_velocities[:, 5, 1]
        ref_dof_vel[:, 9] = local_velocities[:, 6, 1]
        # Torso
        ref_dof_vel[:, 10] = local_velocities[:, 7, 2]
        # Left arm
        ref_dof_vel[:, 11] = -local_velocities[:, 8, 0]
        ref_dof_vel[:, 12] = -local_velocities[:, 8, 2]
        ref_dof_vel[:, 13] = local_velocities[:, 8,  1]
        ref_dof_vel[:, 14] = local_velocities[:, 9,  1]
        # local_velocities[:, 10] # Left Hand
        # Right arm
        ref_dof_vel[:, 15] = local_velocities[:, 11, 0]
        ref_dof_vel[:, 16] = -local_velocities[:, 11, 2]
        ref_dof_vel[:, 17] = local_velocities[:, 11,  1]
        ref_dof_vel[:, 18] = local_velocities[:, 12,  1]
    ...
    def compute_ref_state(self, time: torch.Tensor):
        ...
        frames = torch.minimum(frames, torch.tensor(self.total_frames-1)).cpu()
        
        # Make sure definitely it doesn't go over
        if torch.any(frames >= self.total_frames):
            # TODO, step? or stand
            return
        
        self.ref_dof_pos = self._local_rotation_to_dof_pos(self.motion.local_rotation[frames])
        self.ref_dof_vel = self._local_vel_to_dof_vel(self.motion.global_velocity[frames])
        self.ref_dof_root_ang_vel = self.motion.global_root_angular_velocity[frames].clone().to(device=self.device)
